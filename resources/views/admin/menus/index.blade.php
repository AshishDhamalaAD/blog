            ...
            <x-admin::table :headers="$headers">
                @foreach ($items as $item)
                    <x-admin::table.tr>
                        ...
                        <x-admin::table.td>
                            <x-admin::chip.yes-no :value="$item->isRelatedToArticle()" />
                            @if($item->isRelatedToArticle())
                                <a href="{{ route('articles.show', $item->article->slug) }}" target="_blank" class="inline-block ml-2">{{ $item->article->title }}</a>
                            @endif
                        </x-admin::table.td>
